# Определение токсичных комментариев 

## Описание проекта:

Интернет-магазин запускает новый сервис. Теперь пользователи могут редактировать и дополнять описания товаров, как в вики-сообществах. То есть клиенты предлагают свои правки и комментируют изменения других. Магазину нужен инструмент, который будет искать токсичные комментарии и отправлять их на модерацию. 


## Цель исследования:

Обучить модель классифицировать комментарии на позитивные и негативные. В распоряжении - набор данных с разметкой о токсичности правок.

### Критерии, которые важны заказчику:

* значение метрики качества `f1` не меньше 0.75.

## Ход исследования:

* загрузка данных;
* предобработка данных;
* исследовательский анализ данных;
* обучение моделей, получение предсказаний;
* итоговые выводы


## Признаковое описание данных:

* текст комментария

## Целевой признак:
 
* метка класса: 0 - позитивные комментарии, 1 - негативные

## Файлы с данными:

* датасет с размеченными данными о комментариях

## Используемые библиотеки:

*pandas, numpy, torch, transformers, re, wordcloud, nltk, spacy, gensim, sklearn, optuna, time, tqdm, matplotlib, seaborn, warnings, os*
